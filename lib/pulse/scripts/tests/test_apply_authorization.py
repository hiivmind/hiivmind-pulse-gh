...
def _recorded_summary(rederived, **overrides):
    summary = {
        "binding_id": rederived.binding_id,
...
def test_load_authorization_reads_matching_transformation_entry(tmp_path):
    ...
    assert auth.selection == ("acme/docs",)
...
def test_load_authorization_rejects_empty_selection(tmp_path):
    path = tmp_path / "apply-authorization.yaml"
    path.write_text(
        "authorizations:\n"
        "  plan-sync-doc-patch:\n"
    ...
    with pytest.raises(apply_authorization.AuthorizationError, match="selection must be non-empty"):
        apply_authorization.load_authorization(path, "plan-sync-doc-patch")
...
def test_authorization_digest_changes_when_scope_changes():
    ...
    digest_b = apply_authorization.authorization_digest(_auth(selection=("acme/other",)))
...
def test_authorize_refuses_binding_mismatch():
    ...
    recorded_summary = _recorded_summary(rederived, binding_id="a-different-binding")
...
def test_authorize_refuses_selection_outside_authorization():
    ...
    recorded_summary = _recorded_summary(rederived)
    auth = _auth(selection=("acme/docs",))  # authorization only covers one repo
