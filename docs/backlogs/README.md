...
| **Extract `lib/pulse` as a standalone Python package** — 53 scripts invoked by `{PLUGIN_ROOT}`-relative path from every skill, no real install/version, only 2 of 53 have entry points. Concretizes audit F7's "neutral runtime root" recommendation; mirrors the Nave extraction precedent for the Python half. **Amended:** must ship as an importable SDK (FastAPI-consumable in-process), not just a CLI — the layer-1/driver/skill split already exists in code. **Amended #2:** also ship a first-class MCP server surface over the same SDK, so the fleet program is reachable by any MCP-capable agent, not only a Claude Code plugin session — model-provider agnosticism, prompted by the prior-art research below. | [`2026-08-17-lib-pulse-package-extraction.md`](2026-08-17-lib-pulse-package-extraction.md) |
...
| **Lockstep-catalog remnants** — five small, independent ideas (milestone alignment, changelog rollup, the fleet-membership onboarding cascade, `watch_paths` dead-glob detection, and two unfinished CLAUDE.md/pattern-doc housekeeping tasks) rescued from the retired 2026-07-10 catalog doc before archiving; most of that doc's other entries already shipped under F-series numbers. Low priority, none blocking. | [`2026-08-17-lockstep-catalog-remnants.md`](2026-08-17-lockstep-catalog-remnants.md) |
...
- [`2026-08-17-lib-pulse-package-extraction.md`](2026-08-17-lib-pulse-package-extraction.md) — extract `lib/pulse` as a standalone Python package (SDK + MCP-server-amended), no-spec architectural item
- [`2026-08-17-branch-protection-governance-parity.md`](2026-08-17-branch-protection-governance-parity.md) — fleet-wide branch-protection/ruleset governance parity, reviewed and rescoped from a retired design, no-spec
- [`2026-08-17-lockstep-catalog-remnants.md`](2026-08-17-lockstep-catalog-remnants.md) — five small unbuilt ideas rescued from the retired lockstep-bindings catalog, no-spec
