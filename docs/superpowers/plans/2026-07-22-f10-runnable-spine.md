...
- **Calls the existing `plan_sync.build_result` (`plan_sync.py:366`) — the driver does NOT reconstruct the `compute`/count/proposal loop.** `build_result` already owns counts (`in_sync`, `doc_patches`, `github_patches`, `conflicts`, `excluded`), findings, and the propose-only proposal summaries. The driver assembles the snapshot, passes `mode`, and serialises.
...
**Steps:**
- [ ] **Step 1: Write failing tests** — reuse `test_plan_sync_acceptance.py` fixtures: each merge outcome lands in the right count bucket; conflict withholds base; `dirty_doc`/`local_ahead` exclusion + finding; scheduled-gated transformation yields no runnable proposal; repeat run after sync is a no-op with identical counts; emitted file passes `validate_result.py --kind plan-sync`.
- [ ] **Step 2: Implement** the assembler over `collect` + `plan_sync.build_result`; propose-only, no pen exec, no `gh` writes, no reconstructed decision loop.
...
- **Driver CLI:** `generated_artifact_run.py --workspace <path> [--result <path>] [--mode …]`. Consumes `generated.yaml`; `generated_artifacts.collect` snapshot (`git rev-parse FETCH_HEAD:<path>` tree/blob SHAs). Assembles → `build_result` → validate → write.
...
- **Overlay content collector (`overlay_content.py`):** an explicit, overlay-only collector with an **injected `gh_api`/content-reader seam** (no new runner). Contract: reads the overlay-relevant paths (`.claude-plugin/plugin.json`, `CLAUDE.md`, every `skills/*/SKILL.md`) at an **immutable remote ref** (the evidence snapshot's already-resolved commit SHA, not a branch name); enforces a **per-file byte limit and a total byte budget**; returns explicit **unavailable states** (`missing`, `too_large`, `fetch_error`) per path rather than omitting silently; attaches the result to `evidence["file_contents"]` **only on the opted-in repo's entry**. The neutral snapshot stays content-free (`evidence_snapshot.py:182` unchanged for neutral repos).
...
**Interfaces:**
- **New `periodic` trigger type** in `poll.py`'s dispatch loop (`:581`), alongside `session_poll`/`freshness`. Cadence algorithm, fully specified: a `periodic` workflow declares `trigger: {type: periodic, interval_minutes: N}`; it is **due** when `last_run_at is None` (first-run baseline — due immediately on first poll) **or** `now - last_run_at >= interval_minutes`; the existing `cooldown_minutes` short-circuit (`:577`) still applies as a floor; `last_run_at` is updated by the existing run-ledger path when the surfaced workflow actually runs (not by the poll itself, so a surfaced-but-unrun workflow stays due); a failed run does not advance `last_run_at`. `periodic` sources are **not** repo-scoped (no `REPO_SCOPED_SOURCES` gate). The original Task-5 `branch_heads`/`docs` surfacing is **already implemented** (`poll.py:219`,`:260`,`:310`) — do not rebuild it.
...
**Steps:**
- [ ] **Step 1: Tests** — `poll.py` emits due-workflow state for a `periodic` workflow: due on first poll (baseline), not-due within `interval`, due again after `interval`, cooldown floor respected, `last_run_at` non-advance on skip. Assert all four flipped templates parse and surface.
...
- **Anti-improvisation guard (machine-readable, not grep):** `skills/headless-driver-map.yaml` maps each headless capability → its authorized `uv run …` driver command + validator kind. `test_skills_shell_drivers.py` asserts, for every entry: (1) the mapped driver script exists and exposes the documented CLI (`--help` parses / argparse defines the flags), (2) the driver writes a result on ABORT, (3) it invokes the correct `validate_result.py` kind, and (4) **every headless `SKILL.md` that references a propose/mutate library is present in the map** (so a new bare-signature skill fails the test). This replaces the brittle `module.func(` grep — legitimate explanatory references to `pen_orchestrator.execute`/`object_apply`/`apply_reconcile` in F11 prose no longer false-positive, because the guard checks *presence of an authorized driver mapping*, not the absence of a function name.

**Steps:**
- [ ] **Step 1: Write the acceptance matrix** across all four phases (incl. scheduled-gated cases).
- [ ] **Step 2: Author `headless-driver-map.yaml`** and the mapping-lint test.
