...
API Routing Guide Enhancement Plan
 ...
 New structure (UPDATED - cleaner format):
 ...
 **CLI Command Reference:**
 ...
 | List | `gh domain list` | |
 | Get | `gh domain view` | |
 | Create | `gh domain create` | |

 **Corpus Lookup Guide** (for exact API syntax):

 | API | Endpoints/Mutations | Search Keywords |
 |-----|---------------------|-----------------|
 | REST | `GET /repos/{owner}/{repo}/domain`, `POST /domain`, `PATCH /domain/{id}` | `GET`, `POST`, `PATCH`, `DELETE` |
 | GraphQL | `query { domain }`, `createDomain`, `updateDomain` | `query`, `mutation`, `createX`, `updateX` |

 Format decision:
 - Main table: Clean ✓/✗/⊗ only (no verbose CLI commands inline)
 - CLI Reference: Separate table for commands
 - Corpus Guide: Separate table for API patterns/keywords
 ...
 **Symbol meanings in tables:**
 ...
 ✅ CLI details: Separate table (UPDATED)
 - Main table shows only ✓/✗/⊗ symbols for clean scanning
 - CLI commands in separate "CLI Command Reference" table below
 - Keeps main table focused and readable
 ...
 This enhancement transforms the API routing guide from prescriptive (recommending one method) to descriptive (showing all options). Users will see at a glance which of the 4 methods (gh CLI, REST,
 GraphQL, Web UI) support each operation, enabling them to choose based on their context and constraints.
